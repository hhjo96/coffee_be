...
public class MenuController {
    ...
    public ResponseEntity<BaseResponse<PageResponse<MenuDto>>> getAllV1Projection(Pageable pageable) {
        return ResponseEntity.status(HttpStatus.OK).body(BaseResponse.success("200", "메뉴 목록 조회: 1 성공",
                menuService.getAllV1Projection(pageable)));
    }

    // 목록: 쿼리dsl + 페이징
    @GetMapping("/v2")
    public ResponseEntity<BaseResponse<PageResponse<MenuDto>>> getAllV2DSL(Pageable pageable) {
        return ResponseEntity.status(HttpStatus.OK).body(BaseResponse.success("200", "메뉴 목록 조회: 2 성공",
                menuService.getAllV2DSL(pageable)));
    }
    ...
    public ResponseEntity<BaseResponse<MenuCursorResponse>> getAllV3Cursor(
        ...
        return ResponseEntity.status(HttpStatus.OK).body(BaseResponse.success("200", "메뉴 목록 조회: 3 성공",
                menuService.getAllV3Cursor(cursorId, size)));
    }

    // 단건: @Cacheable
    @GetMapping("/{menuId}/v1")
    public ResponseEntity<BaseResponse<MenuDto>> getOneV1Cacheable(@PathVariable Long menuId) {
        return ResponseEntity.status(HttpStatus.OK).body(BaseResponse.success("200", "메뉴 단건 조회: 1 성공",
                menuService.getOneV1Cache(menuId)));
    }
    ...
    public ResponseEntity<BaseResponse<MenuDto>> getOneV2RedisTemplate(@PathVariable Long menuId, Pageable pageable) {
        return ResponseEntity.status(HttpStatus.OK).body(BaseResponse.success("200", "메뉴 단건 조회: 2 성공",
                menuService.getOneV2Temp(menuId)));
    }
    ...
    public ResponseEntity<BaseResponse<MenuDto>> updateMenuByIdV2(@PathVariable Long menuId, @RequestBody UpdateMenuRequest request) {
        return ResponseEntity.status(HttpStatus.OK).body(BaseResponse.success("200", "메뉴 수정: 1 성공",
                menuService.updatePostByIdV2(menuId, request)));
    }
    ...
    public ResponseEntity<BaseResponse<MenuDto>> updateMenuByIdV1(@PathVariable Long menuId, @RequestBody UpdateMenuRequest request) {
        return ResponseEntity.status(HttpStatus.OK).body(BaseResponse.success("200", "메뉴 수정: 2 성공",
                menuService.updatePostByIdV1(menuId, request)));
    }
